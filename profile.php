...
// Determine which profile to show: self or ?user_id
$currentUserId = getCurrentUserId();
$profileUserId = isset($_GET['user_id']) ? (int)$_GET['user_id'] : (int)$currentUserId;

// Fetch user data and related lists for the profile user
...
$isOwner = ($profileUserId === (int)$currentUserId);
$isFollowing = $isOwner ? false : $userService->isFollowing((int)$currentUserId, $profileUserId);

$stats = $userService->getUserStats($profileUserId);
$achCount = $userService->getAchievementCount($profileUserId);
$reviews = $userService->getRecentReviews($profileUserId, 5);
$watchlist = $userService->getWatchlist($profileUserId, 12);
$watchedMovies = $userService->getWatchedMovies($profileUserId, 18);
$achievements = $userService->getAchievements($profileUserId);
$followersCount = $userService->getFollowersCount($profileUserId);
$followingCount = $userService->getFollowingCount($profileUserId);
?>
    ...
    async function toggleFollow(userId, btn) {
      ...
      try {
        ...
        if (data.success) {
          ...
          btn.innerHTML = following ? '<i class="fas fa-user-plus mr-2"></i>Follow' : '<i class="fas fa-check mr-2"></i>Following';
          btn.className = following ?
            'px-4 sm:px-6 py-2.5 sm:py-3 bg-white text-slate-950 rounded-lg hover:bg-gray-100 transition font-medium shadow-lg text-center' :
            'px-4 sm:px-6 py-2.5 sm:py-3 glass rounded-lg hover:bg-white/10 transition font-medium shadow-lg text-center';
        }
      } finally {
        btn.disabled = false;
      }
                ...
                <div class="flex flex-col sm:flex-row gap-3 sm:flex-shrink-0">
                  ...
                  <?php else: ?>
                    <button
                      onclick="toggleFollow(<?= (int)$profileUserId ?>, this)"
                      data-following="<?= $isFollowing ? '1' : '0' ?>"
                      class="<?= $isFollowing ? 'glass' : 'bg-white text-slate-950' ?> px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:bg-gray-100 transition font-medium shadow-lg text-center">
                      <?php if ($isFollowing): ?>
                        <i class="fas fa-check mr-2"></i>Following
                      <?php else: ?>
                        <i class="fas fa-user-plus mr-2"></i>Follow
